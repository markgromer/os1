...
Service worker `marcus-mobile-v19` adds retry review for an exact draft whose approved delivery failed. The manifest has stable app ID `/mobile.html`, explicit 192x192 and 512x512 PNG icons for `any` and `maskable` purposes, and the mobile acceptance dialog exposes install, new-test, voice, operation approval, and message review controls. Local Playwright at 390x844 verified the dialog layout, installed-context gate, same-session reload recovery, and display-context isolation. Production fetches verified all four PNG dimensions and MIME types. Phone-size production passes verified the exact-target operation and exact-draft message dialogs without bypassing approval. This does not replace the physical installed-Android run.
