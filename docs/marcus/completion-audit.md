...
| Email messaging | Provider verified; live send pending | Resend SMTP is configured and no-send verified for `Marcus <marcus@gromore.media>`. Exact draft `V8uMUUZjiRz1` remains `pending_approval` with blank approval and send timestamps. | Mark must approve and send the displayed acceptance draft; Marcus must retain the provider receipt. |
