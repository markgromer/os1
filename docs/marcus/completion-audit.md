...
| Full-PC project use | Verified within authorized work roots | The scheduled desktop relay declares broad-root authorization for Mark's Documents/OneDrive Documents/Downloads roots, attests exact workspaces, creates projects under `MARCUS_NEW_PROJECT_ROOT`, and launches local Codex with `workspace-write`. | Whole-drive and arbitrary destructive access remain intentionally unavailable; credentials, deletion, and external mutations keep separate controls. |
...
Current local regression passes `140/140`; syntax lint passes for 70 JavaScript files.
