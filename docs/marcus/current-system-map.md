...
`public/obs-marcus.html` is a browser/OBS sidecar for demos and live-call support. It can request microphone capture or browser-supported display/system-audio capture and pass the resulting `MediaStream` into `client/marcus-realtime.js`. It can send pasted context and now polls the authenticated MARCUS browser relay for changed rendered text on Zoom, Skool, Google Meet, Teams, YouTube, and TikTok. Changed text is injected silently through `sendContext()`. Gmail is available for explicit browser inspection but excluded from automatic live-voice forwarding. The relay is bounded to the viewport, strips controls/editable/hidden/off-screen content, stores context only in memory, and does not expose cookies, passwords, form values, profile storage, raw HTML, or unrestricted DOM. This is local browser observation, not a Zoom API connection, and it does not change meeting consent requirements.
