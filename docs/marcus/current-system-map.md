...
Production verification on 2026-08-13 UTC passed the full phone-to-PC route. The scheduled relay advertises the configured broad roots and kiosk mode, hosted Marcus routes exact attested local workspaces through `desktop_codex_with_fallback`, and Scoop Fairies project switching opened the correct VS Code workspace. The `Marcus PC Bridge Demo` operation created and attested a blank Git workspace, launched Codex, resumed the same thread for a correction, and opened a fresh kiosk monitor for a read-only follow-up. Independent project checks pass: 4/4 tests, lint, typecheck, and Wrangler dry-run. The complete Marcus suite passes `140/140`, and syntax lint passes for 70 JavaScript files. `MARCUS_DESKTOP_CODEX_ENABLED=false` disables the desktop path.
