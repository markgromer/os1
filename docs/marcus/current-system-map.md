...
The browser lifecycle closes the media session while the PWA is hidden, resumes with a fresh ephemeral credential, reconnects after network or WebRTC loss with bounded backoff, and refreshes at 55 minutes before the Realtime session limit. A valid durable pairing cookie takes over immediately if a Render restart invalidates a process-bound Live token, even when the stale token is still present in the Authorization header. Service worker cache `marcus-mobile-v17` is live with explicit raster install assets, the acceptance controls, and an exact-target operation approval dialog. The dialog shows project, immutable operation title/target, action, risk, reason, and operation id; approval remains disabled until Mark explicitly checks the authorization statement, and critical actions also require typed strong confirmation. The approval POST uses paired admin authentication and cannot be invoked with the read-only Live token. The acceptance session ID, start time, and coarse platform/display context persist locally for up to two hours; context changes invalidate the ID, and the telemetry queue remains memory-only. Selective Realtime announcements speak only persisted completion, failure, cancellation, approval, blocked, or recovery transitions; ordinary progress polling stays visual. Production phone-size checks on 2026-08-12 confirmed both pending GitHub/Cloudflare targets, disabled-by-default approval controls, zero browser errors/warnings, and a 401 response to an unauthenticated approval attempt. The combined production report passes 10/12 gates; the explicitly approved SMTP acceptance send and physical installed-Android voice acceptance remain open.
