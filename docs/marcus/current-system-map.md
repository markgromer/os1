...
The browser lifecycle closes the media session while the PWA is hidden, resumes with a fresh ephemeral credential, reconnects after network or WebRTC loss with bounded backoff, and refreshes at 55 minutes before the Realtime session limit. A valid durable pairing cookie takes over immediately if a Render restart invalidates a process-bound Live token, even when the stale token is still present in the Authorization header. Service worker cache `marcus-mobile-v19` includes explicit raster install assets, the acceptance controls, an exact-target operation approval dialog, and exact-draft failed-delivery retry. Operation approval remains disabled until Mark checks the authorization statement, and critical actions also require typed strong confirmation. Message delivery remains disabled until Mark checks a separate statement authorizing the exact displayed draft. Both paths use paired admin authentication; direct external-action routes and conversational approval attempts reject a Live token without durable authentication. The acceptance session ID, start time, and coarse platform/display context persist locally for up to two hours; context changes invalidate the ID, and the telemetry queue remains memory-only. Selective Realtime announcements speak only persisted completion, failure, cancellation, approval, blocked, or recovery transitions; ordinary progress polling stays visual. Production phone-size checks on 2026-08-12 confirmed both pending GitHub/Cloudflare targets and email draft `V8uMUUZjiRz1`, disabled-by-default approval controls, zero browser errors/warnings, and 401/reauthentication responses to Live-token-only mutation attempts. The later approved email attempt failed closed at Resend without delivery; physical installed-Android voice acceptance remains open.
...
SMTP is configured through Resend's relay with `Marcus <marcus@gromore.media>`, and Resend shows `gromore.media` as verified. The borrowed credential authenticates to SMTP but is not authorized to send from that domain: approved acceptance draft `V8uMUUZjiRz1` received SMTP `550`, and no email was sent. Marcus now preserves the exact approval and exposes the failed draft for retry after a dedicated sending key scoped to `gromore.media` is installed. Provider-form writes invalidate verification only when that provider's effective configuration fingerprint changes, so an email-key rotation cannot erase unchanged Quo verification. Quo approved-send action `rv1v4_RKB38v` remains durable provider evidence.
...
Local verification on 2026-08-12 passed a real disposable Codex CLI run that created an exact requested file in a new Git repository. The complete repository suite passes `138/138`, and syntax lint passes for 70 JavaScript files. Hosted Marcus enables the desktop adapter by default and routes only exact attested local workspaces to it; GitHub-only jobs retain the configured HTTP or GitHub Actions fallback. `MARCUS_DESKTOP_CODEX_ENABLED=false` disables the desktop path. A restarted desktop relay carrying the broad-root and kiosk settings and a phone-to-PC run are still required before production acceptance is claimed.
