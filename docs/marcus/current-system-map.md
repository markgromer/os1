...
`public/obs-marcus.html` is a browser/OBS sidecar for demos and live-call support. It can request microphone capture or browser-supported display/system-audio capture and pass the resulting `MediaStream` into `client/marcus-realtime.js`. It can send pasted context and polls the authenticated MARCUS browser relay for changed rendered text on Zoom, Skool, Google Meet, Teams, YouTube, and TikTok. Changed text is injected silently through `sendContext()`. Gmail is available for explicit browser inspection but excluded from automatic live-voice forwarding. The relay is bounded to the viewport, strips controls/editable/hidden/off-screen content, stores context only in memory, and does not expose cookies, passwords, form values, profile storage, raw HTML, or unrestricted DOM. This is local browser observation, not a Zoom API connection, and it does not change meeting consent requirements.

The same page is served at `/call-marcus.html` as the normal user-facing name; OBS Studio is not required for ordinary Zoom participation. Its dedicated Realtime `<audio>` element selects an output with `setSinkId()` and automatically prefers Pack45's `Speakers (VB-Audio Virtual Cable)` endpoint, with older `CABLE Input` labels as fallback. This keeps Zoom on headphones while only MARCUS speech goes into VB-CABLE; Zoom uses `CABLE Output` as MARCUS's microphone.

The sidecar also maintains a 20,000-character in-memory meeting transcript window. Every five minutes and on stop it calls `/api/marcus/meeting-notes/checkpoint`. The server reuses `aiTranscriptProposal`, redacts secret-like output, and queues only a concise Markdown summary to the `marcus-meeting-note` desktop action. `desktop-meeting-notes.cjs` confines atomic writes to `docs/marcus/conversations/`. Raw transcript text is not persisted in the Obsidian file or desktop queue.
