...
Status: deployed and accepted for whole-drive `C:\` scope on exact desktop agent `Marks_PC`. Critical operation `op_1HYqnishgglGZQ` recorded Mark's strong confirmation, persisted the policy, read runtime and disk state back, and passed required `pc_access_policy` verification. Physical Android voice acceptance is not an authority source for desktop execution.
...
- `%APPDATA%/M.A.R.C.U.S/desktop-agent.json` as the non-secret scheduled-relay policy file
- `MARCUS_ALLOW_BROAD_WORKSPACE_ROOTS=true`
- `MARCUS_ALLOWED_WORKSPACE_ROOTS` for the exact Windows roots Marcus may inspect
- `MARCUS_FULL_PC_ACCESS` and `MARCUS_PC_ACCESS_ROOTS` for a separately authorized whole-PC scope
- `MARCUS_NEW_PROJECT_ROOT` for from-scratch project creation
- `MARCUS_CODEX_MONITOR_MODE=kiosk` for visible local execution

The installed policy currently reports `scope: full_pc` with the sole root `C:\`. The relay declares eight capabilities: inventory, filename search, bounded text-file read, directory listing, file/folder opening, HTTP(S) URL opening, installed-application listing, and installed-application launch. `GET /api/marcus/pc/capabilities` reports the live declaration. `POST /api/marcus/pc/actions` provides an authenticated deterministic acceptance path, and the same tools are available to main chat and Marcus Live. Production acceptance returned inventory, search, directory, and 545-byte bounded-read evidence from the demo project without relaying a credential-bearing file.
...
Production provider status on 2026-08-13 UTC: outbound code, admin-only mobile configuration, secret redaction, provider authentication, exact-target review, and idempotent sends are deployed. Service worker `marcus-mobile-v22`, one-time pairing, provider configuration, operation approval, required-approval queue, and exact-draft message review pass on Render. The redacted operation summary can advertise pending approvals but cannot execute them. Full message drafts and direct external-action mutations reject a Live token, and Live-token-only conversational approval returns `reauthenticationRequired`. Quo and SMTP both have current verified configuration plus receipt-backed approved-send evidence in `GET /api/marcus/acceptance`; messaging contributes four passing gates. Marcus still must not claim delivery until provider acceptance and receipt evidence exist for that exact draft.
...
- Whole-drive scope: active only for exact agent `Marks_PC` and root `C:\` under the completed durable critical grant. Selected metadata and bounded non-secret text may transit the hosted Render service; credential-bearing content remains blocked.
