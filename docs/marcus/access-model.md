...
Production provider status on 2026-08-12: outbound code, admin-only mobile configuration, secret redaction, provider verification, and mock-provider send acceptance pass locally. Service worker `marcus-mobile-v17`, one-time pairing, the provider and exact-target operation approval dialogs, admin access, and Live-token rejection also pass on Render. The redacted operation summary can advertise one pending approval but cannot execute it; approve/reject routes require the paired durable-admin cookie or server-side admin credential. Quo is configured and verified against the Operations line; approved acceptance action `rv1v4_RKB38v` has provider-accepted `sent` evidence. Resend SMTP is configured for `Marcus <marcus@gromore.media>` and passed no-send authentication verification. Email draft `V8uMUUZjiRz1` remains `pending_approval`. Marcus must not claim a real email was sent until explicit approval, provider acceptance, and receipt evidence exist.
