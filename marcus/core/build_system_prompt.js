...
export const DEFAULT_MARCUS_MODULES = [
...
function insertDynamicModules(baseModules, insertions = []) {
...
async function loadModuleMarkdown(module) {
  const fullPath = path.join(CORE_DIR, module.fileName);
  const content = await fs.readFile(fullPath, 'utf8');
  return normalizeText(content);
}

function resolveOverrideContent(module, uiOverrides = {}) {
...
export async function buildMarcusSystemPrompt(options = {}) {
  ...
  const sections = ['### M.A.R.C.U.S. CORE SYSTEM'];
