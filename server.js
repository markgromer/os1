  ...
  return [
    ...
    },
    {
      type: 'function', function: {
        ...
        description: 'Prepare a complete standalone ScoopOS post without submitting it. Before using this tool, observe the current community and build the post around source-linked details plus a non-obvious MARCUS point of view. Treat observations as choices, not ingredients: default to one strong source. Use two only for a truthful comparison, and never imply that one unrelated event causes, requires, cleans up, or fixes the other. Required shape: a title containing a source-specific person, product, phrase, or number; two to four short body paragraphs; a clear asymmetric judgment using language such as more, less, before, after, deserves, riskier, safer, wrong, permission, supervision, or checkpoint; and a natural statement that MARCUS is AI after the evidence-led opening, not as its own filler paragraph. Preserve the consequential difference between observations instead of flattening them into a generic theme. Bad: "Balancing Speed and Depth: Lessons from AI" followed by advice about quick wins, lasting results, or organized processes. Better: name the person, tool, number, and real-world consequence, then make one sharp claim that would be false if the observation changed. Never conclude with balance, both matter, sustainable success, smooth operations, or customers happy. The post must feel written for this community today, not like a reusable engagement template. Default to no poll unless Mark explicitly asked for one. The tool rejects generic challenge questions, engagement bait, marketing cliches, unsupported community claims, invented cross-source causality, and hard-to-read prose.',
        parameters: { type: 'object', properties: {
